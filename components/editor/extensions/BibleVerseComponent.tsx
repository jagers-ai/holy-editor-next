...
const BibleVerseComponent = React.memo((props: NodeViewRendererProps) => {
  ...
  return (
    <NodeViewWrapper className="bible-verse-wrapper block">
      ...
      <div className="bg-white rounded-md px-2 py-1 my-0 max-w-full border border-transparent">
        {/* 참조 표시 - 상단으로 이동 */}
        {reference && (
      ...
      <div className="h-px my-0 cursor-text bg-white select-none" onMouseDown={(e)=>{const pos=getAfterPos(); if(pos!==null){e.preventDefault(); insertParagraphAt(pos)}}}></div>
    </NodeViewWrapper>
  )
})
